                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        if ($result->num_rows > 0) {
                                            echo "<table class='table table-bordered'><tr><th>Name</th><th>Department</th><th>Description</th><th>Stock</th><th>Updated At</th><th>Action</th></tr>";
                                            while($row = $result->fetch_assoc()) {
                                                ...
                                                if ($rowx['user_type'] == 'Administrator' || $rowx['user_type'] == 'Staff') {
                                                    ...
                                                    echo "<td><button type='button' class='btn btn-danger btn-sm' data-toggle='modal' data-target='#sellItems{$row['id']}' title='Sell'><i class='bi bi-bag-dash'></i></button>";
                                            ...
                                            <div class='modal fade' id='sellItems<?php echo $row['id'] ?>' tabindex='-1' role='dialog' aria-labelledby='sellItemModalLabel' aria-hidden='true'>
                                                <div class='modal-dialog' role='document'>
                                                    <div class='modal-content'>
                                                        <div class='modal-header'>
                                                            <h5 class='modal-title' id='sellItemModalLabel'>Sell Item</h5>
            ...
            <div class="modal-content">
                ...
                <div class='modal-body'>
                    <div class='table-responsive'>
                        ...
                        $result = mysqli_query($conn, "SELECT *, requests.id, requests.datetime as rd FROM requests INNER JOIN products ON requests.product = products.id INNER JOIN faculty ON requests.user_id = faculty.id");
                        if ($result->num_rows > 0) {
                            echo "<table class='table table-bordered'><tr><th>Requested By</th></th><th>Product</th><th>Description</th><th>Stock</th><th>Datetime</th><th>Action</th></tr>";
                            while ($row = $result->fetch_assoc()) {
                                $date_filed = $row['rd'];
                                $time = strtotime($date_filed);
                                $updated = date("F d Y, g:i A", $time);
...
<?php
function addStock($pid, $quantity) {
    global $conn;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["approveStockRequest"])) {
    ...
    if ($fetchRequestResult && $fetchRequestResult->num_rows > 0) {
        ...
        if (addStock($pid, $requestedStock)) {
