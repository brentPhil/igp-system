...
?>
            ...
            INNER JOIN (
                ...
                GROUP BY billing_stall
                                        ...
                                        while ($row = mysqli_fetch_array($result)) {
                                            $date_filed = date("M d, Y | g:i A", strtotime($row['date_filed']));
                                            $fullyPaid = $row['status'] === '1' && $row['amount'] != $row['amount_paid'];
                                            $bgColor = $row['status'] === '1' ? ($fullyPaid ? 'primary' : 'success') : 'secondary';
                                            $paymentStatus = $row['status'] === '1' ? ($fullyPaid ? 'partially paid' : 'paid') : 'pending';
                                            $trimmed_data = str_replace(' ', '', $row['stall_name']);
                                            ?>
                                            <tr data-bs-toggle="collapse" data-bs-target="#<?php echo $trimmed_data ?>" class="collapsed">
                                                <td><i class="bi bi-plus-circle-dotted mr-2"></i><?php echo $row['stall_name']; ?></td>
                                                <td><b>₱ <?php echo number_format($row['amount'], 2); ?></b></td>
                                                <td><?php echo $date_filed; ?></td>
                                                <td>
                                                    <span class="text-light px-3 py-2 rounded-sm bg-<?php echo $bgColor; ?>"><?php echo $paymentStatus ?></span>
                                                </td>
                                                <td class="d-flex justify-content-end">
                                                    <button class='btn btn-success btn-sm mr-2' onclick='generatePDF(<?php echo $row['billing_id'] ?>)'><i class='bi bi-printer'></i></button>
                                            ...
                                            <tr>
                                                <td colspan="5" style="padding: 0 !important;">
                                                    <div id="<?php echo $trimmed_data ?>" class="accordion-collapse collapse" data-bs-parent="#accordion<?php echo $trimmed_data ?>">
                                                        <table class="table table-bordered">
                                                            <tbody>
                                                            <?php
                                                            $allResult = mysqli_query($conn, "SELECT * FROM billing INNER JOIN stalls ON billing.billing_stall = stalls.stall_id WHERE stalls.stall_name = '" . $row['stall_name'] . "'");

                                                            while ($innerRow = mysqli_fetch_array($allResult)) {
                                                                $date_filed_inner = date("M d, Y | g:i A", strtotime($innerRow['date_filed']));
                                                                $fullyPaid_inner = $innerRow['status'] === '1' && $innerRow['amount'] != $innerRow['amount_paid'];
                                                                $bgColor_inner = $innerRow['status'] === '1' ? ($fullyPaid_inner ? 'primary' : 'success') : 'secondary';
                                                                $paymentStatus_inner = $innerRow['status'] === '1' ? ($fullyPaid_inner ? 'partially paid' : 'paid') : 'pending';
                                                                ?>
                                                                <tr>
                                                                    <td></td>
                                                                    <td><b>₱ <?php echo number_format($innerRow['amount'], 2); ?></b></td>
                                                                    <td><?php echo $date_filed_inner; ?></td>
                                                                    <td>
                                                                        <span class="text-light px-3 py-2 rounded-sm bg-<?php echo $bgColor_inner; ?>"><?php echo $paymentStatus_inner ?></span>
                                                                    </td>
                                                                    <td class="d-flex justify-content-end">
                                                                        <button class='btn btn-success btn-sm mr-2' onclick='generatePDF(<?php echo $innerRow['billing_id'] ?>)'><i class='bi bi-printer'></i></button>
                                                                        <button class="btn btn-primary btn-sm mr-2" data-bs-toggle='tooltip' data-bs-placement='top' title='view receipt' data-toggle="modal" data-target="#c_receipt<?php echo ($innerRow["billing_id"]); ?>">
                                                                            <i class="bi bi-receipt"></i>
                                                                            <?php if (!empty($innerRow['c_receipt']) && $innerRow['status'] == 0): ?>
                                                                                <span class="badge badge-danger">1</span>
                                                                            <?php endif; ?>
                                                                        </button>
                                                                        <button class="btn btn-warning btn-sm mr-2" data-bs-toggle='tooltip' data-bs-placement='top' title='view payment details' data-toggle="modal" data-target="#balance<?php echo ($innerRow["billing_id"]); ?>">
                                                                            <i class='bi bi-eye'></i>
                                                                        </button>
                                                                        <button class='deleteBtnBill btn btn-danger btn-sm' data-bs-toggle='tooltip' data-bs-placement='top' title='Remove bill' data-billing_id='<?php echo $innerRow['billing_id'] ?>'><i class="bi bi-trash3"></i></button>
                                                                    </td>
                                                                </tr>
                                                                <?php
    ...
    <script>
        function generatePDF(id) {
            var pdfUrl = 'generate_receipt.php?id=' + id;
